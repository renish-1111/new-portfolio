...
const Experience: React.FC<ExperienceProps> = ({ setCursorVariant, id }) => {
  ...
  return (
    <section id={id} ref={sectionRef} className="py-24 px-6 md:px-20 bg-transparent relative overflow-hidden">
      ...
      <div className="max-w-5xl mx-auto">
        <div className={`mb-16 transition-all duration-700 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <h2 className="text-4xl md:text-6xl font-bold text-white">
            Professional <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-600 inline-block transition-transform duration-300 group-hover:scale-110 group-hover:rotate-2">Experience</span>
          </h2>
        </div>
        ...
        <div className="relative space-y-12 md:space-y-0">
            ...
            {EXPERIENCE.map((job, idx) => (
                <div key={`${job.company}-${idx}`} className={`relative md:flex items-center justify-between group ${idx % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
                    ...
                    {/* Timeline Dot */}
                    <div className={`hidden md:block absolute left-1/2 top-1/2 w-4 h-4 rounded-full border-2 border-yellow-500 bg-transparent transform -translate-x-1/2 -translate-y-1/2 z-10 transition-all duration-500 ${isVisible ? 'scale-100 opacity-100' : 'scale-0 opacity-0'}`} style={{ transitionDelay: `${idx * 200}ms` }}>
                    ...
                    <div className="w-full md:w-5/12">
                        <a 
                            ...
                            className={`relative block glass-card border border-white/5 p-6 transition-all duration-700 hover:border-yellow-500/30 hover:bg-[#151515] group/card flex items-center gap-6 ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-20'}`}
                        ...
                        >
                            ...
                            <div className="w-16 h-16 md:w-20 md:h-20 shrink-0 rounded-xl overflow-hidden bg-white/5 border border-white/10 shadow-inner relative group-hover/card:scale-105 transition-transform duration-500">
                                <div className="absolute inset-0 bg-yellow-500/10 opacity-0 group-hover/card:opacity-100 transition-opacity duration-500"></div>
                                <img src={job.logo} alt={job.company} loading="lazy" decoding="async" width={80} height={80} className="w-full h-full object-cover" />
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                                <h3 className="text-xl md:text-2xl font-bold text-white mb-1 truncate group-hover/card:text-yellow-100 transition-colors flex items-center gap-2">
                                    {job.company}
                                    <svg className="w-4 h-4 opacity-0 -translate-x-2 group-hover/card:opacity-100 group-hover/card:translate-x-0 transition-all duration-500 text-yellow-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
